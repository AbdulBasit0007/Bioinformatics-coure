...
class RandomNetwork(Network):
    ...
    def __init__(self, n_nodes=0, n_edges=0):
        ...
        # make sure that n_edges is not too large [max = (n * (n-1)) / 2]
        max_undirected_edges = int((n_nodes * (n_nodes - 1)) / 2) if n_nodes else 0             # type: int
        ...
        # this calls the initialization method of the parent class (Network), which will add its own class fields
        # (in this case the dictionary of nodes and edge flags) and all Network functions to this class
        Network.__init__(self, undirected=True, allow_self_edges=False)

        # case: the network is empty
        if not n_nodes:
            return
        ...
        # case: no edges to create
        if not n_edges:
            return

        # case (optional): fully connected network
        if n_edges == max_undirected_edges:
            # iterate over all unique pairs of nodes
            for i in range(self.size()):
                for j in range(i + 1, self.size()):
                    # add the edge (the network base class makes sure that the edge is undirected)
                    self.add_edge(i, j)
            return

        # case: normal random network
        # initialise the number of established edges
        established = 0                                         # type: int

        # until all links are set...
        while established < n_edges:
            # ... pick two nodes at random (sample avoids selecting the same node twice)
            ...
            # increase the edge counter so that the creation terminates
            established += 1
